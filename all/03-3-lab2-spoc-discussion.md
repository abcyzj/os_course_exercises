...
特权指令只能在ring0使用，不同特权级能够执行的代码和访问的数据也不同。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

访问门时，需要保证CPL比门的DPL低，比中断服务例程所在段的DPL高。
访问段时，需要保证CPL和RPL都不比段的DPL高。
...
访问数据时max(CPL, RPL) <= DPL

2. 在什么情况下会出现特权级切换？

中断和陷入，或是低特权级代码调用高特权级代码时

3. int指令在ring0和ring3的执行行为有什么不同？

ring3时执行int指令会压入SS和ESP的值，并且切换堆栈，而ring0执行时不会。


4. 如何利用int和iret指令完成不同特权级的切换？

见Lab1扩展练习，大致为构造一个trapframe，然后通过iret指令返回
...
三次（取指和访存的时候都需要进行地址转换，访存可能涉及源操作数和目的操作数）

2. 描述X86-32的MMU地址转换过程；

虚拟地址先通过段机制转换为线性地址，然后通过两级页表转换为物理地址

### 7.4 了解UCORE建立段/页表

1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护模式的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。

第一条指令将cr0寄存器中的PE标记置1，开启保护模式；第二条长跳指令清空预取指令序列，使CPU开始执行32位指令
