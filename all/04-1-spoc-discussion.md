...
从左到右，容量越来越大，访问速度越来越慢，单位存储空间的价格越来越低。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

快速的存储器中存放经常访问的内容，慢的存储器中存储可能用到的内容。各种存储介质都处于繁忙工作的状态，但又不会对系统的性能造成限制。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

遇到过，将暂时用不到的内容存到外存中，用到的时候再读进来。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

将程序中不会同时执行的模块共享同一个内存区域。需要开发者划分程序的模块和指出模块之间的覆盖关系。

2. 什么是交换技术？覆盖与交换有什么不同？

交换技术是将暂时补执行的应用程序放到外存中去，实现多个程序的并发运行。交换是以进程为单位的，且不需要开发者给出模块间的覆盖关系，而覆盖技术是发生在应用程序内部的模块之间的，需要开发者给出模块之间的覆盖关系。

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

可能在一些不需要并发运行程序的系统中有需要，比如科学计算。
...
虚拟存储是将暂时用不到的部分内存块放到外存中去的技术。区别？根本就不相似好吗。好处是可以给用户提供更大的虚拟地址空间，挑战是如何实现高效的页面置换。

2. 虚拟存储需要什么样的支持技术？

需要硬件提供地址转换机制，需要操作系统管理页面的置换。
...
在页式存储管理的基础上，增加请求调页和页面置换的功能。缺页中断处理的功能是调用操作系统提供的缺页处理例程，将外存中相应的页面调入内存中。

 2. 为了支持虚拟页式存储的实现，页表项有什么修改？

页表项需要增加一些额外的标志位，如驻留位、修改位、访问位和保护位等。
...
寻找内存中空闲的物理页面或通过页面置换算法找到一个换出的页面，根据修改位决定是否将其写入外存中并将页表项中的驻留位置为0，然后将需要访问的页面装载入物理内存中，并将相应的页表项的驻留位和物理帧号设置好，最后返回用户程序，重新执行产生异常的指令。


2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

访存时间 * (1 - p) + 缺页异常处理时间 * p，其中p为缺页的概率。
